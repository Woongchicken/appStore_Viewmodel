...
class HistoryAdapter(private val historyEntityList: List<HistoryModel>, private var model : MainViewModel, private val requestSearchScope: CoroutineScope, private val fragment : Fragment) :
    ...
    override fun onBindViewHolder(holder: HistoryViewHolder, position: Int) {
        holder.bind(historyEntityList[position])        // position = ho쇼lder.bindingAdapterPosition
    }
            ...
            binding.comRowid.setOnClickListener { // 클릭한 시간 차를 계산
                if ( !isClicked ) {         // 중복 클릭 방지 변수
                    requestSearchScope.launch {
                        ...
                        Utils.requestSearch(history.searchTerm, model) // 검색
